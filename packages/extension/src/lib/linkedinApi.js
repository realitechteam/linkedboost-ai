 ...
 */
export function getRandomDelay(min = RATE_LIMIT.EXTRACTION_DELAY_MIN, max = RATE_LIMIT.EXTRACTION_DELAY_MAX) {
 ...
 */
export function getTextWithFallback(selectors) {
 ...
 */
export function checkLinkedInLoginStatus() {
 ...
 */
export async function canSyncProfile() {
 ...
 */
export async function recordProfileSync() {
    return new Promise((resolve) => {
 ...
 */
export async function canSuggestReply() {
    return new Promise((resolve) => {
 ...
 */
export async function recordReplySuggestion() {
    return new Promise((resolve) => {
        chrome.storage.local.get([STORAGE_KEYS.REPLY_COUNT_HOUR, STORAGE_KEYS.REPLY_HOUR_START], (result) => {
            const now = Date.now();
 ...
 */
export function getOwnProfileUrl() {
    // Try to get from the profile dropdown or nav
    const profileLink = document.querySelector('a[href*="/in/"][href*="miniProfileUrn"]');
    if (profileLink) {
        return profileLink.href;
    }
    // Try the "View Profile" button
    const viewProfileBtn = document.querySelector('a[href^="/in/"]');
 ...
 */
export function navigateToOwnProfile() {
...
export async function extractProfileData() {
    ...
    if (skillItems && skillItems.length > 0) {
        ...
        }).filter((skill) => !!skill && skill.length > 1 && !skill.includes('endorsement'));
    }
...
//# sourceMappingURL=linkedinApi.js.map