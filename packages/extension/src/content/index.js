...
import { checkLinkedInLoginStatus, isOwnProfile as checkIsOwnProfile, canSyncProfile, recordProfileSync, extractProfileData as extractProfileDataFromAPI, } from '../lib/linkedinApi';
...
async function mountFloatingControls() {
    ...
    root.render(_jsx(FloatingControls, { isAuthenticated: isAuthenticated, isProfileSynced: isProfileSynced, onSyncProfile: () => {
            profileSyncAttempted = false;
            forceExtractAndSyncProfile();
        }, onEnableReply: enableReplyHelper, onAnalyzeProfile: analyzeCurrentProfile, onAnalyzeJob: analyzeCurrentJob, onOpenDashboard: () => {
...
async function handleRateLimitCheck(type) {
    switch (type) {
        case 'profileSync':
...
async function performProfileSync(options) {
...
function injectReplyHelper() {
    ...
    messageInputs.forEach((input) => {
        ...
        // Mount React component
        const root = createRoot(container);
        root.render(_jsx(ReplyHelper, { messageInput: input }));
        // Insert after input
...
function analyzeCurrentProfile() {
    ...
    document.body.appendChild(container);
    const root = createRoot(container);
    root.render(_jsx(ProfileBadge, { profileData: profileData }));
}
...
function showJobMatchResult(result) {
